...
int ShowMenu(void) {
    ...
    ImageFormat(&imOrigin,
                PIXELFORMAT_UNCOMPRESSED_R8G8B8A8);          // Passage de l'image en RGBA 32bit <-- Conseil dans exemples de Raylib
    ...
    while (!fermerFenetre)    // Boucle tant que l'on a pas appuyé sur "Esc" ou la "X"
    {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
            ...
            if (CheckCollisionPointRec(mousePoint, largeurGrilleBoutonPlus)) {
                nombreColonnesGrille = nombreColonnesGrille >= MAX_NOMBRE_COLONNES_GRILLE ? MIN_NOMBRE_COLONNES_GRILLE :
                                       nombreColonnesGrille + 1;
            } else if (CheckCollisionPointRec(mousePoint, largeurGrilleBoutonMoins)) {
                nombreColonnesGrille = nombreColonnesGrille <= MIN_NOMBRE_COLONNES_GRILLE ? MIN_NOMBRE_COLONNES_GRILLE :
                                       nombreColonnesGrille - 1;
            } else if (CheckCollisionPointRec(mousePoint, hauteurGrilleBoutonPlus)) { // Hauteur de la grille
                nombreLignesGrille =
                        nombreLignesGrille >= MAX_NOMBRE_LIGNES_GRILLE ? MIN_NOMBRE_LIGNES_GRILLE : nombreLignesGrille +
                                                                                                    1;
        ...
        for (int i = 0; i < NOMBRE_ITEMS_MENU; i++) {
            ...
            DrawRectangleLines((int) itemMenuRec[i].x, (int) itemMenuRec[i].y, (int) itemMenuRec[i].width,
                               (int) itemMenuRec[i].height,
                               ((i == currentMenu) || (i == sourisSurItemMenu)) ? BLUE : GRAY);
            DrawText(menuText[i],
                     (int) (itemMenuRec[i].x + itemMenuRec[i].width / 2 - MeasureText(menuText[i], 10) / 2),
                     (int) itemMenuRec[i].y + 11, 10,
                     ((i == currentMenu) || (i == sourisSurItemMenu)) ? DARKBLUE : DARKGRAY);
        }

        // Image du menu par defaut
        if (currentMenu != JEU_A_PROPOS && currentMenu != JEU_OPTIONS && currentMenu != JEU_AIDE) {
            DrawText("BIENVENUE DANS GEGK !", 170, 30, 30, DARKGRAY);
            DrawTexture(textureMenu, largeurEcran - textureMenu.width - 60, hauteurEcran / 2 - textureMenu.height / 2,
                        WHITE);
        }
        ...
        // Image du menu Aide
        if (currentMenu == JEU_AIDE) {
            DrawText("AIDE", 400, 30, 30, DARKGRAY);
            DrawTexture(textureAide, largeurEcran - textureAPropos.width - 60,
                        hauteurEcran / 2 - textureAide.height / 2, WHITE);
        }
            // Fonctions options
            if (currentMenu == JEU_OPTIONS) {
                ...
                // Largeur de la grille
                DrawText(TextFormat("Largeur grille : %i", nombreColonnesGrille), largeurGrilleBoutonMoins.x - 250,
                         largeurGrilleBoutonMoins.y + 15, 20, BLACK);
                ...
                DrawText(TextFormat("Hauteur grille : %i", nombreLignesGrille), hauteurGrilleBoutonMoins.x - 250,
                         hauteurGrilleBoutonMoins.y + 15, 20, BLACK);
                ...
                // Choix de l'IA
                DrawText(TextFormat("Prévision IA : %i", optionsIA), IABoutonMoins.x - 250, IABoutonMoins.y + 15, 20,
                         BLACK);
